...
#アップロードページ(ログイン済みのユーザーだけアップロードを許可する。)
...
class SearchView(View):
    ...
    '''
        ...
        if "word" in request.GET:
            ...
            if request.GET["word"] == "" or request.GET["word"].isspace():
                return redirect("tube:index")
...
class SingleView(View):
    ...
    #TODO:コメント投稿も後にSingleModViewと独立させ、未ログインユーザーの投稿は拒否する。
...
#動画の削除も未ログインであれば拒否する。
#TODO:後にアップロードしたユーザーと同一の場合のみ削除を許可させるよう改良する
...
#動画の編集も未ログインであれば拒否する。
#TODO:後にアップロードしたユーザーと同一の場合のみ編集を許可させるよう改良する
